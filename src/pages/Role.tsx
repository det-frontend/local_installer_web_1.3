        ...
        <div className="flex gap-4">
          <div className=" w-[50%] bg-slate-700 p-3 rounded-md">
            {users.map((e: any, index) => (
              <div
                ...
                className={` hover:pb-3 my-3 group duration-500 flex justify-between items-center p-2 rounded cursor-pointer  w-full ${
                  active === index ? "bg-green-600" : "bg-white"
                }`}
                key={`key_${index}`}
              >
                <div>
                  <p>
                    Name : <span className=" font-bold">{e?.email}</span>
                  </p>
                  <p>
                    Station Id :
                    <span className="font-bold"> {e?.stationId}</span>
                  </p>
                  <p>
                    Station No :{" "}
                    <span className="font-bold">{e?.stationNo}</span>
                  </p>
                </div>
                <AiOutlineArrowRight
                  className=" duration-300 group-hover:mr-6"
                  size={30}
                />
              </div>
            ))}
          ...
          <div className="w-[50%] relative rounded-lg p-3 ">
            <div className=" sticky top-[10%] bg-gray-500 p-3 rounded-md  w-full">
              <p>
                Cashier Code:{" "}
                <span className=" font-bold text-2xl">{userObj.name}</span>
              </p>
              <p className="mt-3">
                Phone: <span className=" font-bold">{userObj.phone}</span>
              </p>
              <div className="mt-6">
                ...
                <div className=" mt-2">
                  <button
                    ...
                    className="p-2 w-[100px] mr-2 rounded bg-green-400 relative mt-3"
                  >
                    Add
                  </button>
                </div>
                <div className="flex gap-2 mt-5">
                  {userObj?.roles?.map((e: any, index: any) => (
                    <button
                      ...
                      id={e._id}
                      key={`key_${index}`}
                      className=" p-2 rounded bg-green-400 relative"
                    >
                      <p>{e.name}l</p>
                      <div className=" h-[25px] w-[25px] flex items-center justify-center rounded-full absolute top-[-10px] bg-red-500 right-[-5px]">
                        <p>x</p>
                      </div>
                    </button>
                  ))}
              ...
              <div className=" mt-6">
                ...
                <div className=" mt-2">
                  ...
                  <button
                    className="p-2 w-[100px] rounded bg-green-400 relative mt-3"
